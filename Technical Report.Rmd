...
y1_test = y1[-train]
dim(x1_test)
fit1 <- glmnet(x1_test, y1_test, alpha = .8, lambda = 0.0158)
summary(fit1)
cv_fit$lambda.min
preds <- predict(fit1,newx = x1[train,])
actual <- y1[train]
...
mHIV = lm(y1_test ~ x1_test)
summary(mHIV)
preds2 <- predict(mHIV,newx = x1[train,])
preds2
actual
actual <- y1[train]
rss <- sum((preds2 - actual2) ^ 2)
tss <- sum((actual - mean(actual)) ^ 2)
rsq <- 1 - rss/tss
rsq
```
