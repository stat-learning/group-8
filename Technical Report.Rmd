...
prediction1 = predict.lm(mHIV, Aus)
prediction1 = prediction1[1:26]

ggplot(Aus, aes(x = Year, y = HIV)) +
     geom_point() +
  geom_line(aes(y = prediction1), size = 1)
...
gbm1 <- gbm(HIV ~ Year + Employment + Population + GDP + Coverage + GovtExp + PublicExp, data = temp[train,], distribution="gaussian", n.trees=50, interaction.depth=2)
summary(gbm1)


preds = predict(gbm1, temp[-train,], n.trees = 50, type = "response")
actual <- temp[-train,3]
rss <- sum((preds - actual) ^ 2)
tss <- sum((actual - mean(actual)) ^ 2)
rsqb1 <- 1 - rss/tss
